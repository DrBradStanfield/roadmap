 ...
 * so an interrupted write leaves the record as it was. No network, no model,
 * no telemetry; the record and its .bak siblings are the only files touched.
 */
import {
...
import { loadRecord, PlanError, printable } from './get-plan';
...
// ---------------------------------------------------------------------------
// Opening the record — the write boundary
// ---------------------------------------------------------------------------
 ...
 * JSON array or a bare string, this tool would replace the file with an empty
...
function openRecord(rawPath: string): OpenRecord {
...
// ---------------------------------------------------------------------------
// Writing — backup, then atomic replace
// ---------------------------------------------------------------------------

/** Copy the record beside itself, then prune all but the newest few backups. */
function backup(path: string, now: string): string {
 ...
 * overwrites it).
 */
function writeAtomic(path: string, file: RoadmapFile): void {
 ...
 * whole line of output that reads as this tool's own, so they go too.
 */
