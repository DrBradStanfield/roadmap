...
Duration ~102s, with a shorter cut that drops the website beat, as the current
explainer already does, at ~81s. Brad approved the beats on 2026-09-10; the verify
beat and the open-source line are his additions.

| Beat | On screen | Narration | What changes |
|---|---|---|---|
| 0 (0:00) | Cold open as now, then the GitHub repository page for the last line | "Every new chat starts from nothing. Your health record should not. AI assistants give better answers from structured data. Health by Dr Brad keeps your blood tests, clinic letters, height, weight and more as one structured record, updated whenever you add to it. One file, owned only by you, in your own Dropbox or Google Drive. Every assistant, every chat, the same record. And Dr Brad's protocol behind every suggestion, with the citations. And the whole project is open source, so anyone can read exactly what it does with your data." | The cold open's narration, with "in your own Dropbox" widened to "in your own Dropbox or Google Drive" and the open-source sentence added: `vo.json`, `timing.ts`, and a repo card at the end |
...
| 7 (1:17) | **Two-way sync, about 21s.** The website with today's values, then today's weight typed into it, 91.2 kg, and the cell filling in. Cut to the chat: "What is my latest weight?" and the assistant reads 91.2 back off the file. Then "add today's blood pressure, 118 over 76", and back on the website a refresh drops both values into today's column | "Open the website and the same values are already there, because it is one file, not a copy. Change a value on the website, and your assistant sees it on its next read. Add one in the chat, and it is on the website when you refresh. Two doors, one record." | `Website.tsx` grows the edit, the chat and the refresh. Beats 8 and 9 move out to 98s and 102s. The short cut still drops the whole beat |
