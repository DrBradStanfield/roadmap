...
| `import_documents` (US-35) | Hosted only: extract lab files from the Dropbox folder, then commit what the user accepted | `packages/health-core/src/mcp-tools.ts` + `app/lib/mcp-import.server.ts` + [mcp-build-notes.md](mcp-build-notes.md) | shipped 2026-09-04; verified live 2026-09-05 on the scratch Dropbox |
| `file_results` (US-36) | Both servers: the assistant reads the file, the server checks every row and commits what the user accepted | `packages/health-core/src/mcp-tools.ts` (`fileResultsBundle`) over the same receipt surface | built 2026-09-07; live verification pending |
| Folder nudge (US-37) | Hosted, Dropbox: a read lists folder files not in the record | `app/lib/mcp.server.ts` (`withFolderNudge`) + `folderNudge` in `mcp-tools.ts` | built 2026-09-07; live verification pending |
...
**Keys: the Fly secrets below, all on `health-tool-edu`** (plus the `GOOGLE_DRIVE_CLIENT_ID`/`GOOGLE_DRIVE_SECRET` pair for Drive, §4).

| Secret | Use | Rotation |
| --- | --- | --- |
| `MCP_SEAL_KEYS` | Ordered list of 32-byte keys; `kid` = a short SHA-256 fingerprint of the key (`mcp-seal.server.ts`), not an index. Seal with the **first**, accept **any**. Rotation is one atomic `fly secrets set`: prepend the new key. No window where a machine holds a partial view. | Overlap keeps a leaked key live up to the refresh lifetime (90 d); no-overlap forces every user to reconnect. **Default incident response is no-overlap**, because a leaked seal key is retroactive (§4). |
...
| `import_documents` (US-35) | two-phase: extract (no write) then commit (append + guarded correct) | Reads lab PDFs/images or a ZIP of them (5 MB per file, 20 MB per ZIP, 30 files a day per connection; HEIC not read) from the root of the Dropbox app folder. A Google Drive connection refuses the folder route: `drive.file` cannot see files the user put there. Sends each file to Anthropic's API for extraction at the extract step, before the user has confirmed anything, and returns candidates plus a sealed receipt; we keep none of it, and Anthropic's own API policy (no training on customer content; deletion within 30 days barring usage-policy enforcement or law) is what governs their copy — [commercial terms](https://www.anthropic.com/legal/commercial-terms), [retention](https://privacy.claude.com/en/articles/7996866-how-long-do-you-store-my-organization-s-data). Nothing reaches the record until a separate `commit` call, carrying that receipt, writes accepted values and files documents as metadata-only rows; until then the candidates sit in a pending file in the user's own folder and the receipt expires after an hour. See the receipt design below. `openai/fileParams` was removed 2026-09-07 (US-36): a file dropped into ChatGPT is the assistant's to read. The `file` argument and the fetch behind it were deleted 2026-09-10 (AC12, 27 days early); a call that still carries `file` — a tool list cached before 2026-09-07 — is told to refresh the connector, and nothing is fetched. On Dropbox, `read_record` and `get_plan` also list the folder root and name files not in the record (US-37, the nudge; never an unasked import). |
| `file_results` (US-36) | two-phase: propose (no write) then commit, over the same receipt | The assistant read the file itself and sends one file's rows: `metric`, `printedName`, `value`, `unit` as printed, the collection date. The server resolves the printed name (`CORE_METRIC_ALIASES`, the catalogue) and refuses a row whose name disagrees with its metric; converts the unit through the one table (`lookupUnit`); dry-runs the real append; marks a value under the metric's display floor as a low-confidence question (a unit swap on a floor-0 metric). Same candidates, receipt, commit and 90-day guard as the folder route; the document row carries `metadata.importedVia: assistant` and the client label. Closed-world: no file host, no model. Runs on the stdio server too, with the receipt in process memory. |

**Both servers answer the same six methods from one dispatch** (`packages/health-core/src/mcp-rpc.ts`): `initialize`, `ping`, `tools/list`, `tools/call`, `prompts/list` and `prompts/get`. The prompts are four static starting points (`MCP_PROMPTS`: `summarise_my_plan`, `add_todays_results`, `whats_missing`, `import_my_lab_files`). Each tool definition also carries `_meta['openai/toolInvocation/*']` (the two ≤64-char strings ChatGPT shows while a call runs) and a `cost` (`none|add|correct`) the write budget reads; `cost` is stripped from `tools/list`, so the budget never depends on `annotations`, which are hints a client may not trust. `recordedAt` is REQUIRED on both add tools, the server runs in UTC and cannot know the user's day (US-31 AC11); the future check is UTC+14, the latest day anyone has reached.
...
**Two-phase for the permanent tools (US-36 AC9, 2026-09-07).** On the hosted surface `correct_value`, `update_profile` and `report_feedback` take two calls. A call without `confirm` runs every guard (`expectedValue`, `expected`, the 90-day rule, the health-value scan), charges its full cost, writes nothing, and answers with what it would do and a `confirm` receipt: `{tool, args: sha256, conn, jti, nbf, exp}` sealed as a `'proposal'` blob, not valid before 10 s after issue, expiring at 15 min. The same call with `confirm`, the same arguments (hashed; `report_feedback` hashes the PREPARED text, whitespace collapsed and case folded, so a re-emitted report still confirms) and the same connection then writes, uncharged; the receipt is single-use per machine (the `spentCodes` pattern). What the server enforces, exactly: two calls, identical arguments, same connection, at least 10 s apart, within 15 min. What it cannot: see the user's yes; stop an assistant with a code tool from sleeping 10 s; stop a receipt held into the next turn. What it buys: the proposal lands in the transcript before the write, the client's approval prompt runs twice, and an injected model must emit a second call carrying the receipt. The stdio server ignores `confirm`: a person is watching their own file.
...
never read does not count. It names all **nine** tools, not the four the copy was
...
**Discovery advertises no `scopes_supported`, because nothing enforces scopes.** `mcp-authorize.server.ts` never reads a requested `scope` and every grant carries the same fixed pair, so the menu the two metadata documents used to publish promised a choice the authorize endpoint does not make; it was dropped 2026-09-10. The token response still states the scope the grant holds. No route checks that scope against the tool being called, so every tool a bearer token can reach, it can reach. What actually bounds a call is the write budget and corrections guards in `app/lib/mcp.server.ts` (§2, §3).
...
**Which door.** The app-proxy HMAC guards Brad-funded AI calls from the storefront; an MCP client cannot produce a Shopify signature and these tools cost nothing. OAuth 2.1 bearer with audience validation *is* authentication, and stronger than a shared HMAC secret: per-user, per-client, expiring. **MCP OAuth is the front door for `/mcp`; the HMAC door is untouched elsewhere.** Unauthenticated: only the `.well-known` documents (the two OAuth ones, plus `openai-apps-challenge`, which answers from its own secret).
...
Residual, disclosed: our write is **durable-by-retry**, not serialised. Since 2026-09-09 the browser adapter (`widget-src/src/storage/drive.ts`) runs the same steps 1 and 2 as the server; before that it returned `version: null` and ignored the expected version, so a browser save whose read predated another writer's upload dropped that writer's row with no error on either side (US-10 AC4). What remains on both sides is the check-to-upload gap of one PATCH round trip, which only step 3 can see. On Drive, concurrent writers are **durable-by-retry**, and the guide must say so.
...
- [x] **OPERATOR (Brad), 2026-09-02.** `MCP_SEAL_KEYS` + `MCP_CLIENT_HMAC_KEY` as Fly secrets on `health-tool-edu`; DNS for `mcp.drstanfield.com`; Dropbox console redirect URI.
...
- [x] `npm run test:all` green (83 files, 1692 tests at the time). [x] **OPERATOR (Brad), 2026-09-02.** Live verify against both vendors: Claude and ChatGPT on the web.
...
| Refresh-token size: Google ≤512 B; Dropbox variable, ">1 KB" | Sealed blob ≈500 chars typical, ~1.8 KB worst case **plus padding**, still inside Node's 16 KB header limit. |
