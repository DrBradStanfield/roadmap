...
- AC2: No reminder is ever sent to a typed address until the plan-ready email has **delivered without bouncing**. Bounces arrive in minutes; the cron's 3-day quiet period on typed rows (measured from `created_at`, never `updated_at` — refreshes would reset it) closes the race — including the overdue-at-capture case, where a past-due item would otherwise be sendable the very next morning.
...
- AC8: Abuse bound for `victim@example.com`: **one plan-ready email per address EVER** (the send fires only when the enrolment is new — replaying a capture through the 5/day limiter mails nothing), its unsubscribe link keeps working (AC7 token preservation), labels are allow-listed (AC4), and a complaint (US-22 AC10) suppresses + un-enrols immediately.
...
- **Usage signal:** `reminder_optin` vs `reminder_optout`, both carrying `provider` so the lanes are judged separately; same ~30% kill criterion as US-17. The typed lane's PRIMARY opt-out surface is the email unsubscribe page, so that route records the server-side `reminder_optout` (with the deleted row's provider) — without it this lane's kill criterion was structurally blind. Only NEW enrolments count as `reminder_optin` (refreshes don't inflate the denominator). Bounce rate on new sends stays the estimator for the legacy-~900 backfill decision (US-22 AC8, still Brad's open call).
- The **annual floor (AC6) is computed client-side** with the rest of the schedule (`computeReminderSchedule` — the server stays a dumb scheduler) and applies to BOTH lanes; `annual_checkin` is a first-class `ReminderCategory` (group `annual`, 365-day cooldown, chosen deliberately rather than falling through to the default group).
- Tests: ✅ AC1/AC2/AC4/AC6/AC7 covered 2026-08-14 (health-core floor + label-allow-list suites; server capture/quiet-period/refresh suites; email calendar-section suites). ❌ live verify after deploy: a real typed capture → row lands with provider 'typed' → plan-ready email carries the calendar.
