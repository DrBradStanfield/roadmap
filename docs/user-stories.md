...
**Two typed-address lanes, different risk — do not conflate:**
- **New captures (low risk, build first):** confirm at the moment of capture. Contemporaneous and expected, so complaint risk is minimal.
- **The ~900 existing captures (needs a Brad call):** they gave an email for a report, not for reminders. A bulk "do you want reminders?" mail is a re-permission campaign to people who never asked — real spam-complaint risk on a domain that also carries transactional mail. Preferred shape if we do it: one genuinely useful email (their roadmap + the reminders offer), once, not a bare permission ask. Both lists are `single_opt_in` today, so no confirmation habit exists with these contacts.
...
- AC2: The enrolled email is either provider-verified OR a typed address that completed a confirm-link round trip (sent by Resend). No code path enrolls a typed address without that click — otherwise anyone can make the server email a stranger about their colonoscopy.
- AC2b: A hard bounce on the confirm email means no enrollment, and the address is not added to Klaviyo (invalid addresses cost per-contact spend and dilute ad-audience match rates).
