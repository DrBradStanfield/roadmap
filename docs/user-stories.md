...
- AC3: A transient upstream failure — a 5xx whose body is not our handler's JSON (the Shopify proxy / Fly edge answered, not our route) — is retried once before chat surfaces an error; identical resends are absorbed by the server-side dedup. Added 2026-08-12 from Sentry JAVASCRIPT-REMIX-1M / -22 (sentry-fix loop).
