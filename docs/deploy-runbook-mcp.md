...
procedure. Every `/mcp` path and both OAuth `.well-known` documents return 404 while
`MCP_SEAL_KEYS` is unset, so unsetting it is the kill switch (`openai-apps-challenge`
answers from its own secret regardless). Education app, not
...
**Done since:** `api.chat` and `api.feedback` now call the same `getClientIp(request,
'shopify')` from `local-first-route.server.ts`; `route-helpers.server.ts` no longer has
its own copy.
...
**8a. `import_documents` (US-35). PASSED 2026-09-05** on the scratch Dropbox
(`brad@microvitamin.com`), plus replay, `held_different` replace, the 90-day guard and the
