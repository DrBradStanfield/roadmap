...
**Skin health** (age 18+): Moisturizer (ceramides + nicotinamide), daily sunscreen (regionalized: CeraVe Mineral for US/conventional, Beauty of Joseon for non-US/SI), topical retinoid (with pregnancy caution), advanced treatments (informational). Rendered in own section before supplements, pink accent (#D63384).
...
**Running tests (context-safe):**
- **Always run tests in a Bash subagent** (Task tool, `subagent_type: "Bash"`) to keep verbose output out of the main conversation thread.
- **Target specific test files** when possible instead of running the full suite: `npx vitest run packages/health-core/src/suggestions.test.ts`
- **Full suite before committing**: `npm test` (still in a subagent).
