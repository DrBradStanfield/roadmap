...
6. The weekly product-health loop (docs/loops/product-health/) reports the feature's funnel numbers; adoption evidence feeds the next iteration or a deliberate kill decision.

**The weekly loop is self-improving and its manual lives in the repo**: [docs/loops/product-health/LOOP.md](docs/loops/product-health/LOOP.md) (the scheduled cloud trigger is a thin bootstrap that reads it). The loop runs a Fable orchestrator that fans out `worker` subagents, documents durable findings in docs/loops/product-health/LEARNINGS.md, updates usage-evidence in user-stories.md, and amends its own playbook within hard guardrails (its write scope excludes all production code; the Guardrails section of LOOP.md is Brad-only). Build sessions should append notable usage learnings to LEARNINGS.md too — it's the shared knowledge base the loop reads first.
