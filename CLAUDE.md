...
- **`docs/products.md`** is the **MASTER copy — a real tracked file in THIS repo** (direction INVERTED 2026-08-10 so CI/headless deploys can ship it; claude_business now holds the symlink pointing here). Edit it here directly; claude_business sessions edit the same file through their symlink, which lands as an uncommitted change in this repo — the sweep rule commits it. `scripts/check-symlinks.mjs` blocks any commit that turns it back into a symlink.
...
6. The weekly product-health loop (docs/loops/) reports the feature's funnel numbers; adoption evidence feeds the next iteration or a deliberate kill decision.

**Autonomous loops are a FLEET with a shared constitution**: [docs/loops/LOOP.md](docs/loops/LOOP.md) holds the rules every loop inherits (orchestrator-on-strongest-model + worker fan-out, the entropy constitution — every operative file ≤200 lines/25KB, one-in-one-out, split-don't-grow, monthly pruning — learnings/metrics split, self-improvement protocol, Brad-only Guardrails). Each loop is a thin charter at `docs/loops/<name>/LOOP.md` with its own `LEARNINGS.md` (curated, capped, compacted), `metrics.csv` (append-only — numbers are never prose-summarized), and weekly reports. Fleet index: [docs/loops/REGISTRY.md](docs/loops/REGISTRY.md) — every loop needs a row + a declared success signal before its first run (loops are features; unmeasured loops get killed at the quarterly fleet review). New loops start from `docs/loops/_TEMPLATE.md`. Decision record + research citations: [docs/loops/loop-master-architecture-explanation.html](docs/loops/loop-master-architecture-explanation.html). Build sessions append notable usage learnings to the relevant loop's LEARNINGS.md (respecting its dedup rule: same tag + subsystem → update in place).

**A second, sibling loop covers chatbot retrieval**: [docs/chat-health/LOOP.md](docs/chat-health/LOOP.md) + [LEARNINGS.md](docs/chat-health/LEARNINGS.md). Same shape (Fable orchestrator → `worker` fan-out → self-amending playbook → Brad-only Guardrails), different lane: product-health asks *how do people use the tool*, chat-health asks *does the chatbot retrieve the right knowledge*. It reads production empty-handle turns from `chat_match_events`, fixes at most 5 individual `summary:` lines per run, and **must prove every change with `tools/test-chatbot-matching.ts` before/after and revert on regression** — three separate blanket-rewrite attempts on 2026-08-07 all measurably made retrieval worse, so that rule is the point of the loop, not ceremony. It never edits prompts, clinical content, or production code. Build sessions should append retrieval learnings to its LEARNINGS.md.
...
- **🧹 SWEEP EVERYTHING ON EVERY COMMIT (HARD, Brad 2026-07-31).** When Brad says "commit" or "commit and push" — in ANY thread, about ANY task — commit **every** uncommitted change in this repo, not just the files this session touched. `git status`, stage all of it (tracked modifications *and* untracked new files), commit, push. **Never leave another session's work sitting uncommitted, and never `git stash` it aside to isolate your own.**
  ...
  - **`docs/products.md` is a real file now (2026-08-10)** — sweeping IS the correct handling for product-content edits made from claude_business through its symlink. The guard (`scripts/check-symlinks.mjs`) only blocks the reverse mistake: committing it as a symlink again.
